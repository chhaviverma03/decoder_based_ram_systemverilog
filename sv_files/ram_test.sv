...
virtual ram_if.WRITE_MON wr_mon_vif;
virtual ram_if.READ_MON rd_mon_vif;
...
virtual task body();
$fatal(1,"[TEST] base test called directly - override in child");
endtask
...
env.run(empty_addrs,empty_datas,num_writes,empty_addrs,num_reads,1);
endtask;

endclass:ram_random_test

//==============================================================
//  TEST 2- BLOCK BOUNDRY TEST
//==============================================================
...
// ==================================================
// TEST 8 - Same Address Overwrite Test
// ==================================================
...
class ram_constrained_weighted_test extends ram_base_test;
  ...
  // NEW: was called but never defined anywhere in the original code.
  // Weights addresses toward block boundaries (0x00,0x1F,0x20,...,0x7F) 60% of the time,
  // uniform-random elsewhere. Data is uniform-random. Adjust weights as needed.
  local task gen_weighted_stim(output logic [6:0] addr, output logic [7:0] data);
    logic [6:0] boundary_addrs[8] = '{7'h00,7'h1F,7'h20,7'h3F,7'h40,7'h5F,7'h60,7'h7F};
    int pick;
    if (!std::randomize(pick) with { pick dist {0 := 60, 1 := 40}; })
      $fatal(1, "[TEST] randomize() failed in gen_weighted_stim (pick)");

    if (pick == 0) begin
      int idx;
      if (!std::randomize(idx) with { idx inside {[0:7]}; })
        $fatal(1, "[TEST] randomize() failed in gen_weighted_stim (idx)");
      addr = boundary_addrs[idx];
    end
    else begin
      if (!std::randomize(addr))
        $fatal(1, "[TEST] randomize() failed in gen_weighted_stim (addr)");
    end

    if (!std::randomize(data))
      $fatal(1, "[TEST] randomize() failed in gen_weighted_stim (data)");
  endtask
